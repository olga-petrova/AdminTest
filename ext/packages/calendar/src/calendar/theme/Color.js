/**
 * This is taken from Ext.draw.Color so we don't need to depend on the draw
 * package for calendars. This class should be replaced by a single color 
 * class in the core.
 * @private
 */
Ext.define('Ext.calendar.theme.Color', {
    statics: {
     ...
     * @return {Ext.calendar.theme.Color}
     */
    ...
    createLighter: function (factor) {
        ...
        return Ext.calendar.theme.Color.fromHSL(hsl[0], hsl[1], hsl[2]);
    },
    ...
    toString: function () {
        ...
        if (me.a === 1) {
            ...
            // with the Ext.calendar.theme.Color.RGBA_NONE (which is used a lot for checks)
    ...
    toHex: function (color) {
        ...
        var digits = Ext.calendar.theme.Color.colorToHexRe.exec(color);
    ...
    setFromString: function (str) {
        ...
        if (str === Ext.calendar.theme.Color.NONE) {
            this.r = this.g = this.b = this.a = 0;
            return this;
        }

        if ((str.length === 4 || str.length === 7) && str.substr(0, 1) === '#') {
            values = str.match(Ext.calendar.theme.Color.hexRe);
            if (values) {
        ...
        else if ((values = str.match(Ext.calendar.theme.Color.rgbToHexRe))) {
            r = +values[1];
            g = +values[2];
            b = +values[3];
        } else if ((values = str.match(Ext.calendar.theme.Color.rgbaToHexRe))) {
            r = +values[1];
            g = +values[2];
            b = +values[3];
            a = +values[4];
        } else {
            if (Ext.calendar.theme.Color.ColorList.hasOwnProperty(str.toLowerCase())) {
                return this.setFromString(Ext.calendar.theme.Color.ColorList[str.toLowerCase()]);
            }
        }
    ...
    this.addStatics({
        /**
         * Returns a flyweight instance of Ext.calendar.theme.Color.
         *
         ...
         *
         * @param {Ext.calendar.theme.Color/String/Number[]/Number} [red] Red component (0..255),
        ...
        create: function (arg) {
            if (arg instanceof this) {
                ...
                return new Ext.calendar.theme.Color(arg[0], arg[1], arg[2], arg[3]);
            } else if (Ext.isString(arg)) {
                return Ext.calendar.theme.Color.fromString(arg);
            } else if (arguments.length > 2) {
                return new Ext.calendar.theme.Color(arguments[0], arguments[1], arguments[2], arguments[3]);
            } else {
                return new Ext.calendar.theme.Color(0, 0, 0, 0);
            }
        }
    });
});