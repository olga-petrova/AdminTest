Ext.define('Admin.view.dashboard.DashboardController', {
    ...
    requires: [
        'Ext.util.TaskRunner',
        'Admin.view.dashboard.todowindow.TodoWindow'
    ],
    ...
    addNewToDoItem: function () {
        var toDoWindow = this.lookup('todoWindow'),
            toDoGrid = this.lookup('todoGrid');
        if (Ext.isEmpty(toDoWindow)) {
            toDoWindow = toDoGrid.add({
                xtype: 'todowindow'
            });
        }
        toDoWindow.show();
